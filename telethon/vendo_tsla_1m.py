...
async def main():
    try:
        ...
        # Fetch messages containing "Vendo TSLA" within the last 5 days
        async for message in client.iter_messages(channel, search="Vendo TSLA"):
            if message.date.replace(tzinfo=None) < time_threshold:
                break  # Stop if message is older than 5 days
            if message.text and "Vendo TSLA" in message.text:
                message_info = {
        ...
        if messages_data:
            ...
            df.to_csv('vendo_tsla_5d.csv', index=False, encoding='utf-8')
            print(f"Saved {len(messages_data)} messages to Vendo_tsla_5d.csv")
        else:
            print("No messages found with 'Vendo TSLA' in the last 5 days")
