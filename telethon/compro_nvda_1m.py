...
async def main():
    try:
        ...
        # Fetch messages containing "Compro NVDA" within the last 5 days
        async for message in client.iter_messages(channel, search="Compro NVDA"):
            if message.date.replace(tzinfo=None) < time_threshold:
                break  # Stop if message is older than 5 days
            if message.text and "Compro NVDA" in message.text:
                message_info = {
        ...
        if messages_data:
            ...
            df.to_csv('compro_NVDA_5d.csv', index=False, encoding='utf-8')
            print(f"Saved {len(messages_data)} messages to compro_NVDA_5d.csv")
        else:
            print("No messages found with 'Compro NVDA' in the last 5 days")
